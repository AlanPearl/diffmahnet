...
def plot_mah_hists(flow, data, data_desc=None, flow_desc=None, tfrac=1.0,
                   tobs_ranges=None, mobs_ranges=None, title="",
                   randkey=jax.random.key(0)):
    data_desc = data_desc + " data" if data_desc is not None else "Data"
    flow_desc = flow_desc + " flow samples" if flow_desc is not None \
        else ""
    if mobs_ranges is None:
        mobs_ranges = [(11.45, 11.75), (12.3, 12.7),
                       (13.15, 13.55), (14.2, 14.57)]
    ...
    tgrid_ind = jnp.minimum(jax.vmap(jnp.searchsorted, in_axes=(0, None))(
        tfrac_grid, tfrac), tfrac_grid.shape[1] - 1)
    log_mah = diffmahnet.log_mah_kern(
        mah_params, tgrid, data.logt0)[np.arange(tgrid.shape[0]), tgrid_ind]
    flow_log_mah = diffmahnet.log_mah_kern(
        flow_mah_params, tgrid, data.logt0)[
    ...
    for i_tobs, ax in tqdm.tqdm(enumerate(axes.ravel()), leave=False):
        ...
        for i_mobs in tqdm.trange(len(mobs_ranges), leave=False):
            ...
            if len(all_dat):
                ...
                bins = 70
            color = f"C{i_mobs}"
            ax.hist(flow_hist_dat, bins=bins, linewidth=2,
                    color=color, histtype="step", density=True)
            ax.hist(hist_dat, bins=bins, linestyle="--",
                    color=color, histtype="step", density=True)
...
def plot_mah_residual(flow, data, tobs_ranges=None, mobs_ranges=None, title="",
                      randkey=jax.random.key(0)):
    if mobs_ranges is None:
        mobs_ranges = [(11.45, 11.75), (12.3, 12.7),
                       (13.15, 13.55), (14.2, 14.57)]
    ...
    for i_tobs, ax in enumerate(axes.ravel()):
        ...
        for i_mobs in range(len(mobs_ranges)):
            ...
            if np.any(cut):
                ...
                color = colors[i_mobs]
                ax.plot(tgrid_mean, flow_mean - dat_mean,
                        linewidth=2, color=color)
        ax.plot([], [], alpha=0,
                label=f"t_obs~{np.mean(tobs_ranges[i_tobs]):.1f}")
...
def plot_mah_corner(flow, data, data_desc=None, flow_desc=None,
                    randkey=jax.random.key(0)):
    data_desc = data_desc + " data" if data_desc is not None else "Data"
    flow_desc = flow_desc + " flow samples" if flow_desc is not None \
        else "Flow samples"
    ...
    broadcasted_u = np.tile(
        data.u[:, :, None], (1, 1, tgrid.shape[-1]))
    ...
    fig = corner.corner(
        combined_flow, color="C2", plot_datapoints=False,
        hist_kwargs={**hist_kwargs, "color": "C2"}, labels=labels,
        range=ranges, plot_density=False)
    corner.corner(combined_data, fig=fig, color="C1", plot_datapoints=False,
                  hist_kwargs={**hist_kwargs, "color": "C1"}, labels=labels,
                  range=ranges, plot_density=False)
    fig.axes[1].text(0, .1, data_desc, color="C1")
    fig.axes[1].text(0, 0, flow_desc, color="C2")
    plt.show()


parser = argparse.ArgumentParser(
    description="Plot DiffMahNet validation plots.")
parser.add_argument(
    "SAVE_FILENAME", help="Filename the trained model is saved as.")
parser.add_argument(
...
parser.add_argument(
    "--train_data_dir", type=str, default=TRAIN_DATA_DIR,
    help="Directory containing the training data.")
parser.add_argument(
    "--sats", action="store_true")
parser.add_argument(
    "--not-test", action="store_true")
parser.add_argument(
...
if __name__ == "__main__":
    ...
    data = datatools.DataHolder(
        train_data_dir, is_test=is_test, is_cens=is_cens,
    ...
    plot_mah_hists(flow, data, title="t = t_obs",
                   tobs_ranges=tobs_ranges, mobs_ranges=mobs_ranges,
                   randkey=keys[0])
    plt.savefig(plot_dir / "mah_hist_t1p0.png", bbox_inches="tight")
    plot_mah_hists(flow, data, title="t = 0.6 * t_obs", tfrac=0.6,
                   tobs_ranges=tobs_ranges, mobs_ranges=mobs_ranges,
                   randkey=keys[1])
    plt.savefig(plot_dir / "mah_hist_t0p6.png", bbox_inches="tight")
    plot_mah_hists(flow, data, title="t = 0.3 * t_obs", tfrac=0.3,
                   tobs_ranges=tobs_ranges, mobs_ranges=mobs_ranges,
                   randkey=keys[2])
    plt.savefig(plot_dir / "mah_hist_t0p3.png", bbox_inches="tight")

    plot_mah_residual(flow, data, tobs_ranges=tobs_ranges,
                      mobs_ranges=mobs_ranges, randkey=keys[3])
